...
export class InnerPartComponent implements OnInit, OnDestroy {
  ...
  is_file_type_ok(event) {
    ...
    this.notifier.notify("error", ".تایپ عکس آپلود‌شده پشتیبانی نمی‌شود");
    return false;
  }
  ...
  uploadFile(event) {
    ...
    if (event) {
      ...
      if (this.uploadType == "ermia") {
        this.notifier.notify(
          "error",
          ".لطفا یکی از متد ها را قبل از آپلود عکس انتخاب کنید"
        );
        this.loading = false;
        return;
      }
      ...
      this.picsService.getPics(userId).subscribe(pics => {
        ...
        if (
          this.images.some(pic => {
            if (pic.originalName == this.uploadedImageName) {
          ...
          this.notifier.notify(
            "success",
            ".عکس آپلود شده با موفقیت پردازش گردید"
          );
        } else {
          this.notifier.notify(
            "error",
